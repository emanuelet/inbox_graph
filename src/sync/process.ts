...
function parseGmailMessage(gmailMessage: gmail_v1.Schema$Message): ParsedMessage | null {
  ...
  const parseAddr = (raw: string): Array<{ name: string; email: string }> => {
    if (!raw) return []
  ...
  const from = fromList[0] || { name: fromRaw || 'Unknown', email: fromRaw || '' }
...
export async function processMessagesBatch(messages: gmail_v1.Schema$Message[]) {
  ...
  const threadDocs = [...new Set(parsed.map((m) => m.threadId))].map((id) => ({
    _key: id,
    updatedAt: Date.now(),
  }))
  ...
  const insertEdges = async (collection: string, docs: Array<{ _from: string; _to: string }>) => {
